...
const pilgrimageSchema = new dbs.Schema(
    {
        objectType: {
            type: String,
            default: "holidays",
...
pilgrimageSchema.index({ packageName: 1 });
pilgrimageSchema.index({ destinationCity: 1 });
pilgrimageSchema.index({ packageType: 1 });
pilgrimageSchema.index({ status: 1 });

const Pilgrimage = dbs.model("Pilgrimage", pilgrimageSchema);
