...
const generateHolidayPackagePdf = async (req, res) => {
  try {
    const { id } = req.params;

    // Fetch holiday package
    const holidayPackage = await HolidayPackage.findById(id).lean();
    ...
    res.setHeader(
      'Content-Disposition',
      `attachment; filename="${holidayPackage.packageName.replace(/[^a-z0-9]/gi, '_')}_package.pdf"`
    );
    ...
    doc.font('Helvetica-Bold')
       .fontSize(36)
       .fillColor('white')
       .text(holidayPackage.packageName, 50, 180, {
    ...
    doc.font('Helvetica-Bold')
       ...
       .fillColor('white')
       .text(
         `${holidayPackage.packageDuration.days} Days / ${holidayPackage.packageDuration.nights} Nights`,
         50, 350, { align: 'center' }
       );
    ...
    doc.font('Helvetica')
       ...
       .fillColor('white', 0.8)
       .text(
         `Starting from: ${holidayPackage.startCity} | Package ID: ${holidayPackage.uniqueId}`,
         50, 380, { align: 'center' }
       );
    ...
    doc.font('Helvetica')
       ...
       .text(
         `${holidayPackage.packageDuration.days} Days`,
         70, currentY + 40
       );
    ...
    doc.font('Helvetica')
       ...
       .fillColor(colors.primary)
       .text(
         `${holidayPackage.packageDuration.nights} Nights`,
         70, currentY + 60
       );
    ...
    doc.font('Helvetica')
       .fontSize(14)
       .fillColor(colors.primary)
       .text(holidayPackage.packageType, 70 + cardWidth + 10, currentY + 45);
    ...
    doc.font('Helvetica')
       .fontSize(14)
       .fillColor(colors.primary)
       .text(holidayPackage.startCity, 70, currentY + cardHeight + 55);
    ...
    doc.font('Helvetica')
       .fontSize(12)
       .fillColor(colors.primary)
       .text(holidayPackage.uniqueId, 70 + cardWidth + 10, currentY + cardHeight + 55);
    ...
    doc.font('Helvetica')
       .fontSize(11)
       .fillColor(colors.dark)
       .text(holidayPackage.highlights, 65, currentY + 20, {
    ...
    doc.font('Helvetica')
       .fontSize(10)
       .fillColor(colors.dark)
       .text(holidayPackage.include || 'All major amenities and services', 70, currentY + 45, {
    ...
    doc.font('Helvetica')
       .fontSize(10)
       .fillColor(colors.dark)
       .text(holidayPackage.exclude || 'Personal expenses and optional activities', 90 + sectionWidth, currentY + 45, {
         width: sectionWidth - 40,
         lineGap: 3
       });
    ...
    if (holidayPackage.availableVehicle.length > 0) {
      doc.font('Helvetica-Bold')
         .fontSize(16)
         .fillColor(colors.dark)
      ...
      holidayPackage.availableVehicle.forEach((vehicle, index) => {
        if (currentY > 650) {
          doc.addPage();
          currentY = 50;
        }
    ...
    if (holidayPackage.itinerary.length > 0) {
      holidayPackage.itinerary.forEach((day, index) => {
        if (currentY > 650) {
          doc.addPage();
          currentY = 50;
    ...
    doc.font('Helvetica')
       ...
       .fillColor('white', 0.7)
       .text(
         `Package ID: ${holidayPackage.uniqueId} | Generated on: ${new Date().toLocaleDateString()}`,
...
module.exports = { generateHolidayPackagePdf };